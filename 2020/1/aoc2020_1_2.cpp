...
{    
    ...
    {   
        for (int k=i+1; (k<entries.size()) && found==false; k++)
        {   
            for (int j=k+1; j<entries.size(); j++)
            {
                if (2020 == (entries.at(i) + entries.at(k) + entries.at(j)) )
                {
                    std::cout << "Entry 1: " << entries.at(i) << std::endl;
                    std::cout << "Entry 2: " << entries.at(k) << std::endl;
                    std::cout << "Entry 3: " << entries.at(j) << std::endl;
                    std::cout << "Result: " << entries.at(i) * entries.at(k) * entries.at(j) << std::endl;
                    found=true;
                    break;  
                }
            }
